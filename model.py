...
class ModelInputs(object):
    def __init__(self, config):
        ...
        self.raw_x = tf.placeholder(
            name='x',
            shape=(1, seq_len, config.factors, batch_size),
            dtype=tf.int32)
        self.x = tf.squeeze(self.raw_x, axis=0)

        self.raw_x_mask = tf.placeholder(
            name='x_mask',
            shape=(1, seq_len, batch_size),
            dtype=tf.float32)
        self.x_mask = tf.squeeze(self.raw_x_mask, axis=0)
...
class StandardModel(object):
    ...
    def _get_beam_search_outputs(self, beam_size):
        if beam_size != self.beam_size:
            ...
            self.beam_ys = tf.expand_dims(self.beam_ys, axis=0)
            self.parents = tf.expand_dims(self.parents, axis=0)
            self.cost = tf.expand_dims(self.cost, axis=0)
